...
<div class="space-y-6">
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-2">
      ...
      <button
        class="p-2 border border-border rounded-xl hover:bg-muted transition-colors text-muted-foreground"
      >
  ...
  {:else}
    <div class="glass-card rounded-3xl p-12 text-center">
      <div
        class="bg-muted w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 text-muted-foreground"
      >
        <Search size={32} />
      </div>
      <h3 class="text-xl font-semibold mb-2">No quizzes found</h3>
      <p class="text-muted-foreground">
        Try adjusting your search or upload a new resource to generate a quiz.
      </p>
    </div>
  {/if}
</div>
